...
plotdata_pf_rubins_rule <- plotdata_pf_rubins_rule %>% 
...
save(plotdata_pf_rubins_rule, file="data/plotdata_pf_rubins_rule.RData")
...
### Inspect
```{r}
plotdata_pf_rubins_rule %>% head()

plotdata_pf_rubins_rule %>% 
...
## make it fit the required format
plotdata_ue_rubins_rule <- plotdata_ue_rubins_rule %>% 
  pivot_longer(values_to = "country", cols = country0:country2) %>% 
  mutate(COUNTRY = ifelse(name == "country0" & country == 1, 0, 
                          ifelse(name == "country1" & country == 1, 1,
                                 ifelse(name == "country2" & country == 1, 2, NA)))) %>% 
...
save(plotdata_ue_rubins_rule, file="data/plotdata_ue_rubins_rule.RData")
...
### Inspect
```{r}
plotdata_ue_rubins_rule %>% head()

plotdata_ue_rubins_rule %>% 
  dplyr::filter(sex == 0 & age == 50 & country == 0) %>% 
  mutate(value = paste0(round(fit, 2), " [", round(lower, 2), "; ", round(higher, 2), "]")) %>%
  dplyr::select(tau, value) %>% 
...
## make it fit the required format
plotdata_pi_rubins_rule <- plotdata_pi_rubins_rule %>% 
  pivot_longer(values_to = "country", cols = country0:country2) %>% 
  mutate(COUNTRY = ifelse(name == "country0" & country == 1, 0, 
                          ifelse(name == "country1" & country == 1, 1,
                                 ifelse(name == "country2" & country == 1, 2, NA)))) %>% 
...
save(plotdata_pi_rubins_rule, file="data/plotdata_pi_rubins_rule.RData")
...
### Inspect
```{r}
plotdata_pi_rubins_rule %>% head()

plotdata_pi_rubins_rule %>% 
  dplyr::filter(sex == 0 & age == 50 & country == 0) %>% 
  mutate(value = paste0(round(fit, 2), " [", round(lower, 2), "; ", round(higher, 2), "]")) %>%
  dplyr::select(tau, value) %>% 
  pivot_wider(names_from = tau, values_from = value)  %>% kbl()
